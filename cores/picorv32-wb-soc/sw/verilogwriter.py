class Signal(object):
    ...
    def range(self):
        if self.width > 0:
            ...
            r = self.low
            return f'[{str(r)}:{str(l)}]' if self.asc else f'[{str(l)}:{str(r)}]'
        return ''
...
class Instance:
    ...
    def write(self):
        ...
        if self.parameters:
            ...
            s += ',\n    '.join(
                [
                    f'.{p.name.ljust(max_len)} ({str(p.value)})'
        ...
        if self.ports:
            ...
            s += ',\n    '.join(
                [f'.{p.name.ljust(max_len)} ({str(p.value)})' for p in self.ports]
            )
            s += ')'
        s += ';\n'
        return s
...
class VerilogWriter:
    ...
    def add(self, obj):
        if isinstance(obj, Instance):
            ...
            raise Exception(f"Invalid type!{str(obj)}")

    def write(self, file=None):
        s = ("// THIS FILE IS AUTOGENERATED BY wb_intercon_gen\n"
